 ...
 *      including the effort-only difference;
...
for (const theme of ['dark', 'light']) {
  ...
  const extra = async (path, route) => {
    ...
    if (path === '/api/chat/slots/model' && route.request().method() === 'POST') {
      ...
      await json(route, { ok: true, model: 'claude-opus-4.8', switched: ['chat-1', 'chat-3'], skipped_running: [], unchanged: ['chat-2'], failed: [] })
      return true
    }
    return false
  }
  ...
  await effort().click()
  await page.getByRole('option', { name: 'Configured default · Medium' }).waitFor({ state: 'visible', timeout: 5000 })
  ...
  await panel().getByTestId('bulk-effort-scale').waitFor({ state: 'visible', timeout: 5000 })
