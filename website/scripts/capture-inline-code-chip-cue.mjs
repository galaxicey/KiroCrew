 ...
 *     broken image — each render their in-flow, dismissable "Copy failed".
 *
...
async function main() {
  ...
  // Announced once, from the primed sr-only alert region beside the chip; the
  // bubble's notice was inserted already populated, so it is out of the live tree.
  const hiddenNotice = await h.page.locator('[role="tooltip"] [aria-hidden="true"] [data-testid="md-chip-copy-error"]').count()
  const liveFailAlerts = await h.page.locator('span.sr-only[role="alert"]').filter({ hasText: 'Copy failed' }).count()
  assert(`refused: the bubble notice is visual-only and the primed region announces once (${hiddenNotice}, ${liveFailAlerts})`,
    hiddenNotice === 1 && liveFailAlerts === 1)
  assert('refused: nothing entered the paragraph',
    await envChip.evaluate(el => el.closest('p').querySelector('[data-testid="md-chip-copy-error"], [role="alert"]:not(.sr-only)') === null))
  ...
  assert(`session: chip declares "switch" and paints in the accent (${sessionRest.action}, ${sessionRest.color})`,
    sessionRest.action === 'switch' && sessionRest.color === 'rgb(176, 127, 255)')
  ...
  // --- The in-flow "Copy failed" notice on the title-cued chips: session, path,
  // broken image. Both clipboard layers refuse, as above. ---
  await h.page.evaluate(() => {
    Object.defineProperty(navigator, 'clipboard', {
      configurable: true,
      value: { writeText: () => Promise.reject(new DOMException('denied', 'NotAllowedError')) },
  ...
  await h.page.waitForTimeout(300)
  const inlineNotices = thirdMsg().locator('[data-testid="md-chip-copy-error"]')
  assert(`inline refused: three "Copy failed" notices, one per chip (${await inlineNotices.count()})`, (await inlineNotices.count()) === 3)
  assert('inline refused: every notice reads "Copy failed" and offers a dismiss',
    (await inlineNotices.evaluateAll(els => els.every(el => /Copy failed/.test(el.textContent ?? '') && !!el.querySelector('button')))))
  assert('inline refused: none of them is in a bubble', (await h.page.locator('[role="tooltip"]').count()) === 0)
  assert(`inline refused: the session chip never claims "Copied!" (${await sessionChip.getAttribute('title')})`,
    (await sessionChip.getAttribute('title')) !== 'Copied!')
  assert(`inline refused: the path chip never claims "Copied!"`, (await thirdFile.getAttribute('title')) !== 'Copied!')
  assert(`inline refused: the broken-image chip never claims "Copied!"`, (await brokenChip.getAttribute('title')) !== 'Copied!')
  await shotAround(thirdMsg(), 'inline-copy-failed', 16)
  await inlineNotices.first().getByRole('button').click()
  assert(`inline refused: dismiss removes that one notice (${await inlineNotices.count()})`, (await inlineNotices.count()) === 2)
