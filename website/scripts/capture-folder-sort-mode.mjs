 ...
 *                           arrangement subline; the session menu on the same screen
 *                           is checked quiet
 *   10-failed-read-job-form      the same state beside the job form's folder picker: a
 *                           picker's subline ("listed", not "arrangement") that also
 *                           names the chat sidebar as where "Ask the agent" lives
 ...
 *                           with the same picker-plus-pointer subline as the job form
 ...
 *                           in-menu pair itself (passive notice + the picker's subline,
 *                           and "Ask the agent" as a sibling item -- so no pointer)
 *
...
/** The plain line under a folder-order notice, per surface: the sidebar shows an
 *  ARRANGEMENT; a picker (the session menu's Move to folder, the job form's
 *  folder field, the Command Bar's list) shows a LIST -- and where the picker
 *  has no hand-off of its own it says where the hand-off lives. */
const DETAIL_SIDEBAR = 'Showing your Custom arrangement; retries automatically'
const DETAIL_PICKER = 'Folders are listed in your saved order; retries automatically'
const DETAIL_PICKER_ASK = `${DETAIL_PICKER}. Open the chat sidebar to ask the agent about it.`
...
async function assertNotice(page, testId, label, detail) {
  ...
  if (detail !== DETAIL_SIDEBAR && around.includes('arrangement')) {
    throw new Error(`${label}: a picker's line must not say "arrangement" (${JSON.stringify(around)})`)
  }
...
async function captureFailedRead(browser, base) {
  ...
  await assertNotice(page, 'folder-order-unavailable', 'sidebar', DETAIL_SIDEBAR)
  ...
  await assertNotice(page, 'session-menu-folder-order-unavailable', 'header menu (sidebar collapsed)', DETAIL_PICKER)
  ...
  await openSearchFolders(page)
  await assertNotice(page, 'command-bar-folder-order-unavailable', 'command bar', DETAIL_PICKER_ASK)
  const barOrder = await commandBarOrder(page)
  ...
  await assertNotice(page, 'job-folder-order-unavailable', 'job form', DETAIL_PICKER_ASK)
