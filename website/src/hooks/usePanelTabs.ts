 ...
 *  queries) need no epoch: `purgeDocumentBodiesForRedactionChange` removes their
 ...
 *  `['file-diff', path]` the panel compares them against -- removed, not
 *  invalidated, so a raw body is not shown while revalidating) and the open
 *  tab bodies (`evictDocumentBodies`). Called by the Settings card that made the
 *  change AND by the WebSocket handler for `credential_redaction_changed`, so a
 *  second browser tab purges too -- the two callers share one list on purpose. */
export function purgeDocumentBodiesForRedactionChange(qc: { removeQueries: (f: { queryKey: unknown[] }) => unknown }): void {
  qc.removeQueries({ queryKey: ['file-read'] })
  qc.removeQueries({ queryKey: ['file-diff'] })
  evictDocumentBodies()
}
