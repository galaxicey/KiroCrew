...
export function filterInteractiveModels(
  ...
  return visible.flatMap(model => {
    ...
    seen.add(name)
    // Prefer metadata from an explicitly advertised base model. Pair metadata
    // describes one effort level, so omit it when no base row exists.
    const base = baseModels.get(name)
    return [{ ...(base ?? model), name, ...(!base && name !== model.name ? { description: '', rateMultiplier: undefined } : {}) }]
  })
}
