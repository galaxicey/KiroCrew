 ...
 *  this function exists to refuse. Four shapes make it undecidable, and all are
 *  refused.
 *
 *  One is an insertion that ends where the span lands. The user's own characters run
...
export function locateDictationSpan(
  ...
  if (removed === 0 && inserted === 0) at = span.start         // nothing was edited
    ...
    if (earliest >= span.end) at = span.start                  // the edit is after it
  }
