...
function CredentialRedactionCard() {
  ...
  const set = useMutation({
    ...
    // And DROP every file body THIS document holds (react-query bodies and open
    // tab bodies -- see `purgeDocumentBodiesForRedactionChange`). Other open
    // dashboard documents do the same on the `credential_redaction_changed`
    // push the PUT triggers (useWebSocket), so the purge does not depend on
    // which tab flipped the switch.
    onSettled: () => {
      void qc.invalidateQueries({ queryKey: ['credential-redaction'] })
  ...
  return (
    <SettingsCard>
      ...
      ) : isError && isOwnerGateRefusal(error) ? (
