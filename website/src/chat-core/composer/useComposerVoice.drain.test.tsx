...
describe('useComposerVoice — a drain discard takes the speech and leaves the typing', () => {
  ...
  it('takes nothing back from the draft that follows a send', () => {
    ...
    // which happens to open the same way. Those offsets still resolve against the
    // sent value, so a surviving record would cut into this draft.
    inputRef.current = 'please ship it now'
    act(() => { hook.result.current.cancelVoice() })
    expect(inputRef.current).toBe('please ship it now')
  })
