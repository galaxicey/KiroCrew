...
import { PlainCodeFallback, PlainFilePairHeader } from '../pierre/PlainCodeFallback'
 ...
 *  apart. A rename that keeps its basename reads as the basename alone:
 *  `a.ts → a.ts` would say nothing. */
function sectionTitle(section: PatchSection, fallbackPath: string | null): string {
  const name = section.name ?? fallbackPath
  if (name == null) return ''
  const prev = section.prevName == null ? null : basename(section.prevName)
  return prev != null && prev !== basename(name) ? `${prev} → ${basename(name)}` : basename(name)
}
...
export default memo(function DiffBlock({ code, complete, onFileOpen, pathHint }: { code: string; complete: boolean; onFileOpen?: (path: string) => void; pathHint?: string }) {
  ...
  // cache key and draws no header here, and the plain render prints them
  // verbatim — the one thing "show me the raw diff" promises, and what the
  ...
  const headerControls = () => (
    <span className={`relative z-10 flex items-center gap-1 opacity-0 group-hover/diff:opacity-100 group-focus-within/diff:opacity-100 transition-opacity ${HOVER_NONE_ACTIONS_ROW_CLS}`}>
      ...
      {/* Split/unified is a PIERRE layout option, so omit it while plain mode
          or the streaming stand-in renders the raw patch: over raw text the
          toggle would change nothing the reader can see. It also keeps the
          streaming row at two actions (`max-two-buttons-per-row`). */}
    ...
    <div className="diff-block group/diff rounded-xl border border-border overflow-hidden" title={headerPath ?? undefined}>
      <div className="relative pierre-surface">
        {/* One title row per file, drawn by the block for the block's whole
            life: filename, ± counts, and — on the first row, where Pierre's
            first header carried them — `headerControls`, which act on the
            whole patch. Pierre draws each file's body only. Pierre's own
            header exists only while its renderer holds a highlight result —
            not while its chunk loads, not while the worker pool is starting or
            recovering after a failure, not for a patch that will not parse,
            never in plain mode — so a control slotted into it is gone in every
            one of those states, and a mount of the slot proves nothing about
            the shadow header it projects into. One header per file in every
            state, by construction rather than by detection. The row is the one
            the oversized file-pair card keeps for the same reason, so the two
        ...
        {sections.map((section, i) => (
          <Fragment key={`${section.name ?? ''}:${i}`}>
            <PlainFilePairHeader
              ...
              stats={section}
              renderHeaderActions={i === 0 ? headerControls : undefined}
            />
            {standInForStream
