  ...
  if (variant === 'inline') {
    return (
      ...
      >
        ...
        <span className={`min-w-0 ${messageBelow ? 'basis-full text-[11px] font-normal text-danger/80' : ''} ${messageClassName}`} style={{ overflowWrap: 'anywhere' }} title={messageTooltip}>{message}</span>
        {askAgent && (
          <AskAgentButton
            report={report}
            message={message}
    ...
    >
      ...
      <div className="min-w-0 flex-1 whitespace-pre-wrap" style={{ overflowWrap: 'anywhere' }}>
        ...
        {messageBelow || messageClassName || messageTooltip
          ? <span className={`${messageBelow ? 'block text-[12px] font-normal text-danger/80' : ''} ${messageClassName}`} title={messageTooltip}>{message}</span>
          : message}
