...
/** The core spells the Kiro CLI backend as the empty string — a real value. */
const KIRO_BACKEND = ''
/**
 ...
 */
export function backendUsable(probe: AcpBackendProbe | undefined): boolean {
...
function agentName(probe: AcpBackendProbe): string {
  switch (probe.id) {
    case 'claude': return i18nT('components.kiroPrerequisiteGate.agent_name_claude')
    case 'codex': return i18nT('components.kiroPrerequisiteGate.agent_name_codex')
    case 'opencode': return i18nT('components.kiroPrerequisiteGate.agent_name_opencode')
    case 'pi': return i18nT('components.kiroPrerequisiteGate.agent_name_pi')
    case 'goose': return i18nT('components.kiroPrerequisiteGate.agent_name_goose')
    case 'deepseek': return i18nT('components.kiroPrerequisiteGate.agent_name_deepseek')
    // A harness this build has no name for renders under the server's policy
    // id, exactly as Settings → Agent does, rather than being hidden.
        ...
        <div id={panelId} className="space-y-4 border-t border-border px-4 py-4">
          ...
          {loading && others.length === 0 ? (
            ...
            <p className="text-[13px] text-muted">
              {i18nT('components.kiroPrerequisiteGate.other_agents_unavailable')}
            </p>
          ) : others.length === 0 ? (
            <p className="text-[13px] text-muted">
...
export default function KiroPrerequisiteGate({ children }: { children: ReactNode }) {
  ...
  const backendsQuery = useQuery<{ backends: AcpBackendProbe[] }>({
    ...
    retry: false,
    staleTime: 0,
    // Poll only while a gate screen is what the user sees; once the dashboard
    // is open the answer that let them through is not re-litigated.
    refetchInterval: (query) => {
      const configured = query.state.data?.backends.find(b => b.id === configuredBackend)
      return otherBackendConfigured && backendUsable(configured) ? false : BACKENDS_POLL_MS
    },
  })
