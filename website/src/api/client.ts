...
export const api = {
  ...
  chatSlotsModel: (model: string, skip_running: boolean, reasoning_effort?: string) =>
    post('/api/chat/slots/model', reasoning_effort === undefined ? { model, skip_running } : { model, skip_running, reasoning_effort }).then(j) as Promise<{ ok: boolean; model: string; switched: string[]; skipped_running: string[]; unchanged: string[]; failed: string[] }>,
