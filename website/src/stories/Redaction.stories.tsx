...
import { RedactionAllowedHostsCard } from '../pages/settings/SecurityPanel'
...
function AllowedHosts({ workspaces }: { workspaces: Record<string, string[]> }) {
  const [client] = useState(() => {
    ;(api as unknown as Record<string, unknown>).redactionAllowedHosts = async () => ({ workspaces })
    return new QueryClient({ defaultOptions: { queries: { retry: false } } })
  })
  return (
    <QueryClientProvider client={client}>
      <div className="max-w-[720px] p-4 text-text"><RedactionAllowedHostsCard /></div>
    </QueryClientProvider>
  )
}
