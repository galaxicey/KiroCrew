...
describe('usePanelTabs', () => {
  ...
  it('purgeDocumentBodiesForRedactionChange removes both file caches and evicts the tab bodies', () => {
    const a = renderHook(() => usePanelTabs('slot-a', mock.descriptors))
    act(() => a.result.current.openFile('/clean.md', 'AKIA-raw-while-off', 'slot-a'))
    const removed: unknown[][] = []
    act(() => purgeDocumentBodiesForRedactionChange({ removeQueries: f => { removed.push(f.queryKey) } }))
    expect(removed).toEqual([['file-read'], ['file-diff']])
