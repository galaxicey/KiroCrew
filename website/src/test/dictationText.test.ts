...
describe('finding a written span in a value the user has edited', () => {
  ...
  it('refuses when the user\'s own typing ends where the span lands', () => {
    ...
    // And typing AFTER the run -- the ordinary thing to do during a long wait -- is
    // untouched: those characters are pinned in the unchanged prefix.
    const ship = { start: 7, end: 11, text: 'ship', trail: '', replaced: '' }
    expect(locateDictationSpan('please ship', 'please ship today', ship)).toBe(7)
  })
})
