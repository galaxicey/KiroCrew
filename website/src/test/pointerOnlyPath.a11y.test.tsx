...
// ── Expansion while a recalled message owns the composer ───────────────────
//
// A token pairs with its block by `seq` alone and `nextSeq` restarts at 1 after
// a send, so a fresh draft's `[ Paste #1 ]` and the `[ Paste #1 ]` text an older
// sent message kept are the same literal. Expanding then injects the DRAFT's
// content into someone else's recalled message and discards the saved draft,
// because any value change exits history mode and clears the draft ref.

describe('expansion refuses while a recalled history message owns the composer', () => {
  const onSend = vi.fn()
   ...
   *  before that frame runs is silently reset to 0 -- which would leave the
   *  chord with no token under it and pass these tests for the wrong reason. */
  const flushFrames = () => act(async () => {
    await new Promise<void>(r => requestAnimationFrame(() => r()))
    await new Promise<void>(r => requestAnimationFrame(() => r()))
  })

  /** ArrowUp recalls the newest sent message. The draft must already hold the
   *  token: the prune effect drops every block whose token is absent from the
   *  value, so a harness that starts empty has NO block by the time the recall
   *  happens and the collision this describes cannot occur at all. */
  const draft = `my draft ${token}`

  async function recallAndAim(offset: number, length: number) {
    const ta = composer()
    expect(ta.value).toBe(draft)
    ta.setSelectionRange(0, 0) // atStart, so ArrowUp recalls despite a non-empty draft
    fireEvent.keyDown(ta, { key: 'ArrowUp' })
    await flushFrames()
    expect(ta.value).toBe(recalled) // the recall really happened
    const start = recalled.indexOf(token)
    ta.setSelectionRange(start + offset, start + offset + length)
    // The aim survived the recall's frame, so the routes below really are
    // pointed at the recalled token.
    expect(ta.selectionStart).toBe(start + offset)
    expect(ta.selectionEnd).toBe(start + offset + length)
    return ta
  }

  it('refuses the keyboard route on a recalled token', async () => {
    renderWithProviders(<Harness initial={draft} />)
    const ta = await recallAndAim(0, token.length)
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    await flushFrames()
    // Unchanged: the draft's content was NOT injected into the recalled message.
    expect(ta.value).toBe(recalled)
    expect(ta.value).not.toContain('DRAFT CONTENT')
  })

  it('refuses the mouse route on a recalled token', async () => {
    // The pointer path resolves the same seq collision, so the guard has to sit
    // where both routes meet rather than on the keyboard chord alone.
    renderWithProviders(<Harness initial={draft} />)
    const ta = await recallAndAim(2, 0)
  ...
  it('refuses the touch route on a recalled token', async () => {
    touchEnv.touch = true
    renderWithProviders(<Harness initial={draft} />)
    const ta = await recallAndAim(2, 0)
  ...
  it('still expands a token in an ordinary draft, so the guard is not a blanket refusal', async () => {
    // The opposite failure mode: a guard that refused everywhere would pass every
    // test above while breaking the feature. No recall here, so history mode is
    // idle and the same selection must expand.
    renderWithProviders(<Harness initial={token} />)
    const ta = composer()
