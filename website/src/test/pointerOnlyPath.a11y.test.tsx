...
describe('collapsed paste token expands by keyboard, not only by pointer', () => {
  ...
  function PasteHarness({ initial, sent }: { initial: string; sent?: string[] }) {
  ...
  /** Select exactly the token, which is what Shift+ArrowRight produces. */
  const selectToken = (ta: HTMLTextAreaElement, start: number) =>
    ta.setSelectionRange(start, start + token.length)

  beforeEach(() => { onSend.mockClear() })

  it('expands the token the selection covers', () => {
  ...
  it('expands a token that does not start the draft', () => {
  ...
  it('does NOT expand from a collapsed caret on a token edge', () => {
    // On macOS Option+ArrowDown is native paragraph navigation, and the caret is
    // snapped out of token interiors so it parks on edges. Expanding from a
    // collapsed caret would make ordinary navigation destroy the block.
    renderWithProviders(<PasteHarness initial={`hi ${token}`} />)
    const ta = composer()
    ta.setSelectionRange(3, 3) // the token's start edge
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    expect(ta.value).toBe(`hi ${token}`)
  })

  it('does NOT expand when the selection only partly covers the token', () => {
    renderWithProviders(<PasteHarness initial={token} />)
    const ta = composer()
    ta.setSelectionRange(0, token.length - 2)
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    expect(ta.value).toBe(token)
  })

  it('stays out of the send key: Alt+Enter on a selected token still sends', () => {
    // `Enter` without Shift IS the send binding in the default mode, which
    // Alt+Enter satisfies. The expansion chord must not take that key.
    renderWithProviders(<PasteHarness initial={token} />)
    const ta = composer()
    selectToken(ta, 0)
...
// ── A recalled message cannot borrow the draft's paste block ───────────────
//
// A token pairs with its block by `seq` alone and `nextSeq` restarts at 1 after
// a send, so a draft's `[ Paste #1 ]` and the `[ Paste #1 ]` text an older sent
// message kept are the same literal. The draft's blocks are therefore PARKED for
// as long as a recall owns the textarea. Scope, not a flag: a flag keyed on "is
// history mode active" is cleared by the user's own edit to the recalled text,
// while the colliding block is still live.

describe('a recalled message cannot borrow the draft paste block', () => {
  const onSend = vi.fn()
  const block: PasteBlock = { id: 'p1', seq: 1, lines: 40, content: 'DRAFT CONTENT\n...40 lines...' }
  const token = '[ Paste #1 · 40 lines ]'
  /** An older sent message that kept the same token literal. */
  const recalled = `earlier message ${token}`
  /** The draft must already hold the token: the prune effect drops every block
   *  whose token is absent from the value, so a harness that starts empty has NO
   *  block by the time the recall happens and no collision is possible. */
  const draft = `my draft ${token}`

  function Harness({ initial = draft }: { initial?: string }) {
    const [v, setV] = React.useState(initial)
    const [blocks, setBlocks] = React.useState<PasteBlock[]>([block])
    return (
      <ChatInput
  ...
  /** The recall parks the caret in a requestAnimationFrame, so a selection set
   *  before that frame runs is silently reset to 0 -- which would leave a route
   *  with no token under it and pass these tests for the wrong reason. */
  const flushFrames = () => act(async () => {
    await new Promise<void>(r => requestAnimationFrame(() => r()))
    await new Promise<void>(r => requestAnimationFrame(() => r()))
  })

  /** ArrowUp from the start of a non-empty draft recalls the newest sent message. */
  async function recall() {
    ...
    await flushFrames()
    expect(ta.value).toBe(recalled) // the recall really happened
    return ta
  }

  /** Point a route at the recalled text's token and assert the aim stuck. */
  function aim(ta: HTMLTextAreaElement, text: string, offset: number, length: number) {
    const start = text.indexOf(token)
    ta.setSelectionRange(start + offset, start + offset + length)
    expect(ta.selectionStart).toBe(start + offset)
    expect(ta.selectionEnd).toBe(start + offset + length)
  }

  it('leaves the recalled token inert for the keyboard route', async () => {
    renderWithProviders(<Harness />)
    const ta = await recall()
    aim(ta, recalled, 0, token.length)
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    await flushFrames()
    expect(ta.value).toBe(recalled)
    expect(ta.value).not.toContain('DRAFT CONTENT')
  })

  it('leaves the recalled token inert for the mouse route', async () => {
    renderWithProviders(<Harness />)
    const ta = await recall()
    aim(ta, recalled, 2, 0)
  ...
  it('leaves the recalled token inert for the touch route', async () => {
    touchEnv.touch = true
    renderWithProviders(<Harness />)
    const ta = await recall()
    aim(ta, recalled, 2, 0)
  ...
  it('stays inert after the recalled text is EDITED, which ends history mode', async () => {
    // The case a flag cannot cover. Editing the recall clears history mode and
    // discards the draft, so a guard keyed on that flag goes inert while the
    // colliding block is still live. Parking the blocks with the draft is what
    // holds here, on every route.
    renderWithProviders(<Harness />)
    const ta = await recall()
    const edited = `${recalled} and a note`
    fireEvent.change(ta, { target: { value: edited } })
    await flushFrames()
    expect(ta.value).toBe(edited) // history mode has ended by now

    aim(ta, edited, 0, token.length)
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    await flushFrames()
  ...
  it('gives the blocks back with the draft, so the draft token expands again', async () => {
    // Parking must not be one-way: walking back past the newest sent message
    // restores the draft, and its own token has to work afterwards.
    renderWithProviders(<Harness />)
    const ta = await recall()
    ta.setSelectionRange(recalled.length, recalled.length) // atEnd, so ArrowDown walks forward
    fireEvent.keyDown(ta, { key: 'ArrowDown' })
    await flushFrames()
    expect(ta.value).toBe(draft) // the draft is back

    aim(ta, draft, 0, token.length)
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    await flushFrames()
    expect(ta.value).toContain('DRAFT CONTENT') // and its block came back with it
  })

  it('still expands in an ordinary draft, so parking is not a blanket refusal', async () => {
    // The opposite failure mode: parking that never unparked would pass every
    ...
    fireEvent.keyDown(ta, { key: 'ArrowDown', altKey: true })
    await flushFrames()
    expect(ta.value).toContain('DRAFT CONTENT')
  })
})
