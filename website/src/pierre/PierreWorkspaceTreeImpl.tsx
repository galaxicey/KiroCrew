...
export function PierreWorkspaceTreeImpl({ projectDir, onFileOpen, onAddToContext, searchQuery, mode = 'all', selectedPath, persistExpansion = false }: {
  ...
  if (ready && paths.length === 0) {
    ...
    // itself -- "the workspace folder", so it does not read as the host's own
    // "Couldn't load the file tree" (a failed request; this listing answered) --
    // and it is actionable the way the host's failed-listing notice is
