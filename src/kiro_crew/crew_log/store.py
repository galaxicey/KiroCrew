...
class CrewLog:
    ...
    def append_if(
        ...
        *,
        ...
        means it declined; nothing was written and the file is identical.
        ...
        The precondition runs while a cross-process lock is held, so it must be
        bounded and must not write to this log. Reads are safe: ``iter_from``,
        ``page`` and ``resolve`` take neither the lease nor this lock, and the lease
        is refcounted per process, so a nested claim from the same process is not
        contention.
        """
        return self._append(
            type,
    ...
    def _append(
        ...
        *,
        ...
        with _open_lock(_lock_path(self._kind, self._id)):
            ...
            # leaves the file exactly as a format refusal above does.
