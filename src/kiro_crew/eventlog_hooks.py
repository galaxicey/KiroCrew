...
#: Key under which the config reconcile's ``observed`` carries the stamp of the
#: config state its correction was read from. Not a projection key: what that
#: correction has to be re-checked against is the CONFIG, and no projection holds
#: one.
_OBSERVED_CONFIG_STAMP = "config_stamp"


def _config_is_still_live_at(values: dict, observed: dict) -> bool:
    """Is the config state this correction was read from still unchanged?
    ...
    config the caller is still holding. The roster fold is last-wins per field,
    so appending the older values regresses it durably -- an append-only log with
    no compaction has nothing to reopen it until the next read.

    The projection cannot answer this, which is why *values* goes unread. By the
    time the comparison is made the fold ALREADY shows the saved value -- that
    difference is exactly what the comparison found -- so asking whether the fold
    moved since then sees no change and lets the regression through. The question
    belongs on the config side: the correction is valid only while the state it
    was read from is unchanged.

    The stamp must be PAIRED with the read it describes -- taken just before it,
    by whoever performed it, which is what ``load_config_with_stamp`` does in one
    call. A stamp taken later, where the append happens, can certify a state that
    was already replaced when the values were read, which is the accepting
    direction and precisely the regression. Taken just before, the error goes the
    other way: a replacement mid-read refuses a correction the next read makes
    again.

    ``config_read_stamp`` pairs the file fingerprint with the config cache's
    invalidation generation, so a replacement the fingerprint cannot distinguish
    -- same device, inode, timestamps, size and mode -- is still caught by the
    generation every successful write advances. It covers both config files
    together rather than one member, so an edit to any member refuses this one's
    correction too. That is the safe direction for the same reason -- a refusal
    costs one skipped correction, a wrong acceptance is permanent. An ``observed``
    carrying no stamp matches nothing and so refuses every correction, rather
    than silently comparing against nothing.
    """
    from kiro_crew.config.loader import config_read_stamp

    try:
        return config_read_stamp() == observed.get(_OBSERVED_CONFIG_STAMP)
    except Exception:
        logger.debug("config reconcile could not re-stamp the config state", exc_info=True)
        return False


def reconcile_member_config(
    slug, name, agent_cfg, roster_view, *, config_stamp
) -> "list[str] | None":
    ...
    *config_stamp* is the stamp of the config state *agent_cfg* was read from,
    taken just before that read -- ``load_config_with_stamp`` produces the pair
    in one call. It is required rather than defaulted because a caller that
    cannot say which config state its correction came from cannot have that
    correction applied safely, and a default would let the next caller skip the
    question silently.

    The append is conditional: it goes through
    ``append_closer_if_still_applies`` with ``_config_is_still_live_at``,
    re-stamped under the per-slug write lock, so a save that landed after the
    caller's read refuses the correction instead of being overwritten by it. A
    refusal is a normal outcome, not a failure, and the next read compares afresh.
    ...
    try:
        ...
        appended = svc.append_closer_if_still_applies(
            ...
            still_applies=_config_is_still_live_at,
            # What the caller decided on is the config state when it read it; the
            # predicate re-stamps and compares against this.
            observed={_OBSERVED_CONFIG_STAMP: config_stamp},
        )
...
def reconcile_members_at_startup(cfg, state, autonudge_svc) -> int:
    ...
    * write CLOSERS for durable facts the log still believes are open but the
    ...
    try:
        ...
        # The config-reconcile step needs a stamp PAIRED with the read its values
        # come from, and *cfg* is the caller's -- the gateway's boot config, handed
        # to this sweep as a background task. A stamp taken here would certify a
        # config state that a save could already have replaced since that read,
        # which is the accepting direction. So this sweep performs its own paired
        # read, and reconciles config against it.
        #
        # *cfg* still decides WHO is swept and answers every other question here
        # (slug identity, the patrol and slot closers), because membership is not
        # what the stamp verifies: a member added since the gateway booted is
        # reconciled by the first roster read instead of this sweep.
        from kiro_crew.config.loader import load_config_with_stamp

        config_stamp, stamped_config = load_config_with_stamp()
        stamped_agents = getattr(stamped_config, "agents", {}) or {}
        ...
        for name in agents:
            ...
            try:
                ...
                # The paired read's copy of this member, not the caller's: the
                # stamp describes that read, so only its values can be verified
                # against it. A member the paired read does not know gets no
                # config correction; its closers below still run.
                stamped_cfg = stamped_agents.get(name)
                if stamped_cfg is not None:
