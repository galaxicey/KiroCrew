...
def render_pr_summary(observation: Mapping[str, Any]) -> str:
    ...
    if (
        ...
        # readings and only the pair says which one this is.
        parts.append(f"remarks={len(carried)} of {total} within the fetch horizon")
...
def pr_evidence(
    ...
    *,
    ...
    budget. ``pr_state`` and ``pr_checks`` are summaries of typed facts the watch's
    own record still holds, so the budget sheds them first; a ``pr_comment`` or
    ``pr_review`` carries prose nothing else has a copy of.
