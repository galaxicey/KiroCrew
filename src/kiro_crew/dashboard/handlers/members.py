...
async def api_member_perpetual_set(request: web.Request) -> web.Response:
    ...
    ON with an active loop: emit the critical start audit, repair its owner
    admission, then return the current record. Audit or trust-write failure: 503.
        ...
        if existing is not None:
            if existing.active:
                ...
                if not await _audit_perpetual_owner_arm(existing, slot_key, caller):
                    return _perpetual_error(
                        "audit log unavailable — owner authorization not recorded",
                        "perpetual_on_failed",
                        503,
                    )
                try:
