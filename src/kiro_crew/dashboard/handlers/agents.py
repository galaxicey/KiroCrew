            ...
            if request.method == "PATCH" and patch_body is not None:
                ...
                async with _get_config_lock():
                    ...
                    if "skills" in patch_body:
                        ...
                        # under the lock, below. The URIs steer the merge's reorder.
                        _applied, unknown, mapped_uris = await loop.run_in_executor(
                            discovery_executor(),
                            apply_skill_mapping,
                            data,
                            f,
                    ...
                    def _locked_overwrite() -> dict[str, Any]:
                    ...
                    try:
                        written = await asyncio.to_thread(_locked_overwrite)
                    except CapabilityError as exc:
                        return web.json_response(
                            {"error": exc.code, "code": exc.code}, status=exc.status
                        )
                ...
                if "skills" in patch_body:
                    ...
                    # reply as its next state. Same catalog walk as the GET, off the loop
                    # for the same reason.
                    mapped = await loop.run_in_executor(
                        discovery_executor(),
                        agent_skill_keys,
