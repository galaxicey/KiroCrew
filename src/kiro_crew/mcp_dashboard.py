    ...
    return [
        ...
        },
        {
            ...
            "description": (
                ...
                "its own. A channel agent (a Channels session, or a session driven from "
                "a messaging channel such as Slack or Discord) cannot bind a folder at "
        ...
        },
        {
            ...
            "description": (
                ...
                "or a session driven from a messaging channel such as Slack or Discord) "
        ...
        },
        {
            ...
            "description": (
                ...
                "Nor may either move a folder to where those sessions would inherit "
                ...
                "A channel agent (a Channels session, or a session driven from a "
                "messaging channel such as Slack or Discord) may move the person's "
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "chat_folder_update":
        ...
        if d.get("error"):
            ...
            # channel caller (a Channels agent's ``channel:`` key, or a session
            # driven from a messaging transport -- the endpoint's own predicate
            # decides) has none -- the endpoint refuses its create-time binding
            # with the same code -- so pointing it at chat_folder_create would
            # send it to a second refusal.
            hint = (
                ...
                and not _is_channel_caller_key(caller_key)
                else ""
            )
