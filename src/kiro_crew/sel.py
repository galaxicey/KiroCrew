...
class SecurityEventLog:
    ...
    def _chain_lock(self, *, kind: str = "append") -> Iterator[None]:
        ...
        process; when the directory cannot be created at all,
    ...
    def _chain_lock_path(self) -> Path:
        ...
        nor hold it out from under the writers. :meth:`_chain_lock_target`
        decides what to lock when that directory cannot be created at all.
        """
    ...
    def _chain_lock_target(self) -> tuple[Path, bool]:
        ...
        Resolved at acquire time from one fact -- whether the sidecar's
        directory can be created -- and never from this process's own key
        migration state. That distinction is the whole point: a directory's
        permissions are a property of the filesystem, so every writer on the
        install reads the same answer and locks the same inode, whereas the
        key's location varies per process and would fork the chain.
