...
#: wrong, while losing the oldest loses one earlier burst's record alone.
RECEIPT_MAX_OWED = 4
...
class QueueReceipt:
    ...
    * a body written to the bubble may quote only the lines that arrived at that
    ...
    def terminalize(self, body: str) -> None:
        ...
        del self.owed_bodies[:-RECEIPT_MAX_OWED]
        self.lines = []
...
class ReceiptQueue:
    ...
    async def _write_record(self, receipt: QueueReceipt) -> bool:
        ...
        if surface is None:
            return False
        if not await self._edit(surface, receipt.msg_id, bodies[0]):
            if not await self._post_record(surface, bodies[0]):
                return False
        del bodies[0]
        while bodies:
            if not await self._post_record(surface, bodies[0]):
