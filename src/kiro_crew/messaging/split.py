...
def _redact_only_the_rejoined_span(text: str, redactor: Callable[[str], str]) -> str | None:
    ...
    It carries the same promise to callers as the wider
    :func:`_flush_whitespace`, and by the same test rather than by assertion: the
    loop returns only once the form with EVERY run removed reads clean, so a
    ...
    cannot make that promise, which hands the caller back its wider answer -- the
    case being a key that only the canonical reading sees, which collapsing
    whitespace does not assemble and so no span of the literal text names.
    ...
    """
    repaired = text
    for _ in range(len(_WHITESPACE_RUN.findall(text)) + 1):
...
def repaired_for_delivery(chunks: list[str], redactor: Callable[[str], str]) -> str | None:
    """``None`` when *chunks* are safe to deliver one message each, else a repair.

    For a caller that bounds chunks AFTER this module graded them. A fixed-width
    slice of an oversized chunk is a boundary nothing graded, and each slice is
    posted as its own message, so the seam the splitter closed reopens at the very
    last step. Grading the sequence here means the caller does not have to know
    how the grade works, only that it has one to run.

    The returned text is safe to bound AGAIN by any rule the caller likes, because
    the repair is only returned once the form with every whitespace run removed
    reads clean -- so no re-cut can uncover a key. That is what makes one pass
    enough rather than a grade after every slicing step.
    """
    if not _rejoins_a_key(chunks, redactor):
        return None
    joined = "".join(chunks)
    repaired = _redact_only_the_rejoined_span(joined, redactor)
    return repaired if repaired is not None else _flush_whitespace(joined, redactor)
...
def _cut_where_no_key_rejoins(
    ...
    4. only if that span cannot be named, flush the whitespace and redact.
