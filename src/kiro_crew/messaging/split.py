...
def repaired_for_delivery(chunks: list[str], redactor: Callable[[str], str]) -> str | None:
    ...
    rendering assembles -- the answer is the canonical form of the whole text, which
    gives up markup and keeps every space and every break. That is
    ...
    if repaired is not None:
        return repaired
    return redact_for_display(canonicalize_display(joined), redactor)[0]
...
def bounded_for_delivery(
    ...
    """*chunks*, graded once more and cut back inside *budget* if the grade repairs.
    ...
    delivered = [piece for chunk in chunks for piece in (cutter(chunk, budget) or [chunk])]
    repaired = repaired_for_delivery(delivered, redactor)
    if repaired is None:
        return delivered
    return cutter(repaired, budget) or [repaired]
...
def _cut_where_no_key_rejoins(
    ...
    return chunks if chunks is not None else [repaired]
