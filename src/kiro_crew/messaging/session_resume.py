...
class ResumeBinding:
    ...
    later, which is what keeps a natively accepted entry native. ``observed`` is
    the binding the conversation held when the pin was taken or last validated.
    ...
    @classmethod
    def native(cls, observed: str | None = None) -> "ResumeBinding":
        """The conversation's own session, whatever it resumes later."""
        return cls(resumed_key=None, observed=observed)

    @classmethod
    def from_route(cls, route: "RoutingDecision") -> "ResumeBinding":
        """Pin a fresh message to the routing decision just taken for it."""
        return cls(resumed_key=route.resumed_key, observed=route.resumed_key)

    @classmethod
    def for_replay(cls, marker: str | None) -> "ResumeBinding":
        """Pin a drained entry to the key it recorded; an empty marker is native."""
        return cls(resumed_key=marker or None, observed=None, is_replay=True)
    ...
    def validated(self, current: str | None) -> "ResumeBinding":
        """The same pin, recording the binding it was just validated against."""
        return replace(self, observed=current)
