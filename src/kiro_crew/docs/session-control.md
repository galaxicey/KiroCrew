...
folder with a binding of its own moves freely; nor may either move a folder to
where its sessions would inherit different steering directories (a folder's
`steering_dirs` are read into every chat filed beneath it). A channel agent (a
Channels session, or a session driven from a messaging channel such as Slack or
Discord) cannot bind a folder at all — neither at create nor with
`chat_folder_update` — and, though it may move the person's folders, is held to
the same rules on a move: not across a binding, not across declared steering.
