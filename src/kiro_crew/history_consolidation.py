...
class _WriteGate:
    ...
    * :meth:`boundary` -- before every awaited write batch or transaction is
    ...
    #
    # The three verbs that EMBED before they land (semantic, episodic, lesson)
    # also hand ``admit`` INTO the store: an embedding is seconds of model
    # inference between the admission here and the row, and a ``!incognito``
    # landing in that gap must refuse the row, not follow it. The store asks the
    # hook again once its write transaction is open and before its commit
    # (``VectorMemory._write_semantic`` / ``write_episodic``), and a raise there
    # unwinds through the store's own rollback -- the same contract as the member
    # transaction's ``admit``. ``test_consolidation_write_gate.py`` pins both:
    # the pass-through by syntax, the refusal-at-the-row by an embedder that
    # flips the mode.

    def set_semantic(self, store: Any, /, *args: Any, **kwargs: Any) -> Any:
        self.admit("the semantic write")
        return store.set_semantic(*args, **kwargs, admit=lambda: self.admit("the semantic write"))
    ...
    def write_episodic(self, store: Any, /, *args: Any, **kwargs: Any) -> Any:
        ...
        return store.write_episodic(*args, **kwargs, admit=lambda: self.admit("the episodic write"))

    def write_lesson(self, store: Any, /, *args: Any, **kwargs: Any) -> Any:
        self.admit("the lesson write")
        return store.write_lesson(*args, **kwargs, admit=lambda: self.admit("the lesson write"))
