...
#: stderr signatures that mean "the forge refused this for budget reasons".
_RATE_LIMIT_RE = re.compile(
    r"rate limit|secondary rate|abuse detection|too many requests|\b429\b",
    re.IGNORECASE,
)
...
class PrObservation:
    ...
    def as_facts(self) -> dict[str, object]:
        ...
        Every key here has a reader, and nothing this record does not carry is kept
        anywhere else either: a count of the calls a tick spent describes the fetch
        rather than the subject, and a field no consumer reads is a claim the reading
        cannot back. What the window had left stays on the transport, which is the
        one place it is read -- to yield a call on a nearly-spent window.
        """
    ...
    def call(self, args: list[str]) -> _Response:
        ...
        while attempt < _MAX_ATTEMPTS:
            ...
            try:
                proc = run_gh(
                    [resolve_gh(), *args],
                    timeout=min(_GH_TIMEOUT_SECS, max(1.0, self.budget_left)),
