...
class WhatsAppRenderer(Renderer):
    ...
    async def _render_live(self) -> None:
        ...
        while self._sealed_count < len(chunks) - 1:
            ...
            if joins_to_a_credential(rendered[index], "".join(rendered[index + 1 :]), _redact_all):
                return
    ...
    async def on_done(self, stop_reason: str = "") -> None:
        ...
        if repaired is not None:
            ...
            # The repair can introduce a placeholder of its own, and the notice
            # below counts ``chunks``. What ships from here is ``pending``, so the
            # tail it replaces has to be the tail the tally reads, or a reply whose
            # only redaction came from the repair announces none.
            chunks = chunks[:start] + pending
