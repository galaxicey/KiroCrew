...
from kiro_crew.messaging.split import chunk_utf8_bytes, repaired_for_delivery
...
def _bounded_chunks(content: str) -> list[str]:
    ...
    repaired = repaired_for_delivery(chunks, _redact_all)
    if repaired is None:
        return chunks
    return chunk_utf8_bytes(repaired, WEBEX_MAX_TEXT) or [repaired]
