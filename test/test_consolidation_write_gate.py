...
class TestTheRowReAsksAfterTheEmbedding:
    ...
    def test_a_refused_episodic_write_at_the_cap_evicts_nothing(
        ...
        def _admit_then_tighten(site: str) -> None:
            # Sites: the verb (1), the open transaction (2), before the commit (3).
            asked.append(site)
            real_admit(site)
            if refused_at == "before the commit" and len(asked) == 2:
                sm.set_flag(live_key, "incognito", True)
        ...
        assert len(asked) == (2 if refused_at == "before the first mutation" else 3), asked
    ...
    def test_the_semantic_row_re_asks_inside_its_transaction(self, tmp_path, monkeypatch):
        """No embedding precedes a semantic row, so the pin is the count: the gate is
        asked at the verb, once the transaction is open, and before the commit."""
        live_key, log, sm, sessions = _channel_thread(tmp_path, monkeypatch)
        gate = _make_consolidator(log, sessions=sessions)._write_gate(live_key)
        store = self._real_store(tmp_path)
        asked: list[str] = []
        ...
        def _counting_admit(site: str) -> None:
            asked.append(site)
            real_admit(site)

        monkeypatch.setattr(gate, "admit", _counting_admit)
        assert gate.set_semantic(store, "pref.editor", "vim", 1.0, "consolidation") is None
        assert (
            asked == ["the semantic write"] * 3
        ), f"set_semantic did not re-ask the gate inside its transaction: admissions={asked}"
