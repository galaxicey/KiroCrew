...
class TestUpdateReasoningEffortValues:
    ...
    def test_selected_dynamic_level_survives_cold_restore(self, monkeypatch, tmp_path):
        ...
        assert not (tmp_path / "validated_effort_levels" / "minimal").exists()

        register_reasoning_effort_values(["minimal"])
        self._mod._remember_reasoning_effort_for_restore("minimal")
        marker = hashlib.sha256(b"minimal").hexdigest()
        assert (tmp_path / "validated_effort_levels" / marker).is_file()
    ...
    def test_concurrent_marker_writes_share_the_durable_cap(self, monkeypatch, tmp_path):
        ...
        assert (
            sum(
                path.is_file()
                for path in (tmp_path / "validated_effort_levels").iterdir()
                if path.name != ".lock"
            )
            == 1
        )
    ...
    def test_marker_directory_is_hidden_from_agent_io(self):
        ...
        leaf = "validated_effort_levels"
        assert leaf in _CREW_HIDDEN_LEAVES
        assert leaf in _CREW_PRECREATE_HIDDEN_DIR_LEAVES
        assert leaf in _CREW_SECRET_LEAVES
