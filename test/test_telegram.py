    ...
    def test_a_replayed_entry_for_a_restricted_resumed_session_is_not_spooled(
        ...
        async def _go() -> None:
            await d.handle_message(
                ...
                binding=ResumeBinding.for_replay("dashboard:restricted"),
            )
        ...
        async def _go() -> None:
            ...
            for i in range(52):
                await d._enqueue_with_receipt(
                    ResumeBinding.native().at(key), 7, f"m{i}", origin=_tg_origin()
                )
    ...
    def test_no_receipt_when_turn_finished_before_enqueue(self) -> None:
        ...
        async def _go() -> bool:
            return await d._enqueue_with_receipt(
                ResumeBinding.native().at("telegram:kirocrew:direct:7"),
                7,
                "late message",
                origin=_tg_origin(),
            )
        ...
        async def _go() -> None:
            ...
            for msg in msgs:
                assert await d._enqueue_with_receipt(
                    ResumeBinding.native().at(self._KEY),
