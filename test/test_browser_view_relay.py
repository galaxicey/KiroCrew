...
def _stub_authorize(
    outcome: str, port: int | None = None
...
async def test_dead_upstream_answers_502(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with _dead_upstream_port() as dead_port:
        monkeypatch.setattr(
            browser_cli_view, "relay_authorize", _stub_authorize("ok", dead_port)
        )
        client = TestClient(TestServer(_relay_app(), host="127.0.0.1"))
        await client.start_server()
        try:
...
def test_dead_upstream_port_cannot_be_taken_by_a_concurrent_worker() -> None:
    ...
    with _dead_upstream_port() as dead_port:
        ...
        # Released only at the end of the block, so the port is reusable after.
        reclaimed = dead_port
    with socket.socket() as after:
        after.bind(("127.0.0.1", reclaimed))
...
def test_the_stub_factory_matches_the_real_relay_authorize_signature() -> None:
    ...
    assert [
        name for name, p in real.parameters.items() if p.kind is p.KEYWORD_ONLY
...
async def test_ownership_unproven_is_audited_with_its_own_reason(
    ...
    monkeypatch.setattr(browser_view_relay, "sel", lambda: recorder)
    monkeypatch.setattr(
        browser_cli_view, "relay_authorize", _stub_authorize("ownership_unproven")
    )
    client = TestClient(TestServer(_relay_app(), host="127.0.0.1"))
    await client.start_server()
    try:
