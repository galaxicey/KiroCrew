...
class TestTheTerminalKeepsWhatItCan:
    ...
    def test_the_wider_flush_is_what_this_replaces(self) -> None:
        """For contrast, on the same text: the flush keeps neither."""
        flushed = _flush_whitespace(self._safe(), _default_redactor)
        assert "\n" not in flushed
        assert "**" not in flushed

    def test_two_keys_keep_the_prose_between_them(self) -> None:
        """One span per key, so the paragraphs between them are not collapsed.

        A span bracketing BOTH keys also brackets everything between, and closing
        that up would destroy text the credentials have nothing to do with.
        """
        middle = "middle prose here"
        text = f"{self.BODY}{HEAD} {TAIL}\n{middle}\n{KEY[:12]} {KEY[12:]}\n{self.BODY}"
        safe = redact_for_display(text, _default_redactor)[0]
        repaired = _redact_only_the_rejoined_span(safe, _default_redactor)
        assert repaired is not None
        assert middle in repaired
        assert repaired.count("\n") == safe.count("\n")
        collapsed = _WHITESPACE_RUN.sub("", canonicalize_display(repaired))
        assert _default_redactor(collapsed) == collapsed

    def test_a_key_only_the_rendering_reveals_defers_to_the_flush(self) -> None:
        """No span of the literal text names it, so the wider answer is the caller's.

        Emphasis holds the halves apart, and removing whitespace does not assemble
        the key -- only rendering the markup away does.
        """
        markup = f"{KEY[:4]}**{KEY[4:]}**"
        assert _default_redactor(markup) == markup
        canonical = canonicalize_display(markup)
        assert _default_redactor(canonical) != canonical
        assert _redact_only_the_rejoined_span(markup, _default_redactor) is None
...
class TestTheLastStepIsGradedToo:
    """A caller that bounds chunks AFTER the splitter graded them regrades.
    ...
    def test_the_repair_fires_on_that_sequence(self) -> None:
        assert repaired_for_delivery(self.SLICED, _default_redactor) is not None

    def test_a_clean_sequence_needs_no_repair(self) -> None:
        assert (
            repaired_for_delivery(["ordinary text here", "and more of it"], _default_redactor)
            is None
        )

    @pytest.mark.parametrize("cap", [5, 9, 13, 20, 40])
    def test_the_repair_survives_any_re_cut(self, cap: int) -> None:
        """Why one grade at the end is enough: the repair is safe to bound again."""
        repaired = repaired_for_delivery(self.SLICED, _default_redactor)
        assert repaired is not None
