...
class TestEveryVerdictLaneRetainsAWithheldVerdict:
    ...
    def test_every_withheld_arm_retains_before_it_returns(self, lane: str) -> None:
        ...
        # Every retention names the status it is retaining for and hands over the
        # body file plus the write command, which is where a PATCH's body lives.
        for i in withheld:
            call = body[i - 1].strip()
            status = body[i].strip().split()[1]
            assert call.startswith(f"{RETAIN} {status} "), (call, body[i].strip())
