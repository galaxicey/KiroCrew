...
class TestQueueReceipt:
    ...
    async def test_a_burst_grows_ONE_receipt_rather_than_many_bubbles(self) -> None:
        ...
        await d._enqueue_with_receipt(
            ResumeBinding.native().at(d._session_key(_EMAIL)), inbound, "first"
        )
        await d._enqueue_with_receipt(
            ResumeBinding.native().at(d._session_key(_EMAIL)), inbound, "second"
        )
    ...
    async def test_the_receipt_is_edited_never_deleted_on_cancel(self) -> None:
        ...
        await d._enqueue_with_receipt(
            ResumeBinding.native().at(d._session_key(_EMAIL)), inbound, "held"
        )
...
class TestDrainIdentity:
    ...
    async def test_each_queued_message_is_answered_under_its_own_sender(self, monkeypatch) -> None:
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), first, "mine")
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), second, "and mine")
    ...
    async def test_one_senders_burst_still_collapses_into_a_single_turn(self, monkeypatch) -> None:
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), first, "first")
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), second, "second")
        sessions._busy = False
    ...
    async def test_the_receipt_is_flipped_in_the_chat_that_holds_its_bubble(
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), queuer, "held")
        sessions._busy = False
    ...
    async def test_the_enqueued_entry_records_the_senders_own_origin(self) -> None:
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), sender, "hello")
...
class TestTeamsSharesTheQueueWithOtherTransports:
    ...
    async def test_a_foreign_entry_is_set_aside_and_this_turn_still_answers(
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), mine, "mine")
    ...
    async def test_the_drain_is_registered_and_answers_with_no_opening_envelope(
        ...
        assert await d._enqueue_with_receipt(ResumeBinding.native().at(key), mine, "answer me")
        sessions._busy = False
    ...
    async def test_the_drain_wakes_the_owner_of_an_entry_it_set_aside(self, monkeypatch) -> None:
        ...
        assert await d._enqueue_with_receipt(
            ResumeBinding.native().at(key), self._from(_EMAIL, "CONV", "act-1"), "mine"
        )
...
class TestABusyResumedSession:
    ...
    async def test_a_false_enqueue_retry_stays_on_the_pinned_session(self) -> None:
        """Admitted for session A; during the awaited steer the conversation rebinds
        to B and A's turn ends, so the enqueue finds no turn and the message is
        retried as a fresh turn. The retry must not route again -- that is how it
        ...
        async def _steer(text: str) -> bool:
            ...
            sessions._busy = False  # and A's turn ended, so the enqueue finds no turn
            return False

        provider.steer = _steer  # type: ignore[method-assign]
        ran: list[str] = []
        d._run_turn = AsyncMock(  # type: ignore[method-assign]
            side_effect=lambda *a, **kw: ran.append(
                kw["binding"].resumed_key if "binding" in kw else kw.get("resumed_key") or ""
            )
        )

        await d.handle_message(_inbound("and the weather?"))

        assert ran != ["dashboard:chat-B"], "the retry re-routed into the new session"
        assert ran == ["dashboard:chat-A"], "the retry did not run where it was pinned"
        assert d._session_resume.route.await_count == 1, "the retry routed again"
        assert not any(
            "Dropped a queued message" in body for _, body, _ in client.sent
        ), "a live message was dropped"
    ...
    async def test_a_queued_resumed_entry_replays_in_that_session(self) -> None:
        ...
        ran: list[str] = []
        d._run_turn = AsyncMock(  # type: ignore[method-assign]
            side_effect=lambda *a, **kw: ran.append(kw["binding"].resumed_key or "")
        )
