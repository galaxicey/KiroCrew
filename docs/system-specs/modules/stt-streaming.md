...
froze. It also takes the dictated run
...
Which side the edit fell on is not always decidable, and four shapes make it so. One
is an insertion that ends exactly where the span lands: the user's own characters run
...
taking it deletes a word they authored. A deletion is not in doubt the same way,
