...
private is not disclosed to it either, durable write or not. The three verbs that
EMBED before they land (`set_semantic`, `write_episodic`, `write_lesson`) also hand
`admit` INTO the store as its `admit` hook: an embedding is seconds of model
inference between the admission at the verb and the row, so the store asks the
hook again once its write transaction is open (after every embed and read,
before the first mutation) and before its commit, and a raise there unwinds
through the store's own rollback -- a `!incognito` landing during a lesson's
embedding refuses the lesson AT the row rather than after it. The V1 episodic
cap eviction is one of those mutations: `write_episodic` makes room
(`_evict_for_episodic_cap`) INSIDE that transaction, after the admission and
before the insert, so a refused row unwinds the tombstones with it and the
store stays byte-identical -- evicted ahead of the transaction, an existing
episode was gone for a row that never landed. A member (V2) store publishes a span in one
