...
`SessionManager.mapped_sid`, the slot's session mapping read without pruning. It has two
producers. The dashboard runner latches the read itself, before each of its allocations, by
whichever allocation observes it first. The channel dispatchers take the ALLOCATION's own
capture instead: `SessionAllocationService` reads `mapped_sid` inside the registration's
critical section -- under its lock, before the new sid is mapped -- and exposes it as
`SessionManager.allocation_predecessor(key)` for the caller to consume after the claim. The
difference is a race the dashboard's latch still carries: a caller that read the mapping
before its own `get_or_create` can be suspended inside the allocation, waiting for the turn
permit, while a concurrent turn on the same key allocates an intermediate session and has it
recycled, and its value then names the store before that intermediate one. One limit is recorded rather than worked around: an
