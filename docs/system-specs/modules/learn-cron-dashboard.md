...
refuse. ON on an already-active row emits the same critical `autonudge_start`
audit-or-deny event as a new owner arm, then re-records its owner entry under the
same slot lock before answering success. Audit or trust-write failure returns
503. This repairs an outside resume or invalidated record without rewriting the
live loop. ON on a STOPPED legacy loop is an
