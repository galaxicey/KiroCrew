...
The predicate's memory is bounded, because it retains external ids: each remembered id must fit a snowflake's length, the admitted-destination history and the DM pairing store each carry a named cap, and the DM store evicts least-recently-used. Dropping history may only ever cost a send, never authorize one, so exceeding the admission cap is recorded and makes the final fall-through refuse instead of falling back to the admits-anybody test.

Which authorities apply follows from the route, classified before either is read. A route **naming a channel in its path** reads both. A route naming none, whose **caller supplies the destination**, reads the roster only: a button press's own reply carries its destination in an opaque token, so the path cannot yield it while the dispatcher holds it directly, and the roster is synchronous and in-memory, so re-reading it costs nothing against the ~3 second deadline those replies carry, while the `channels` ceiling is a blocking governance read and stays off that path. A route with neither reads nothing. **Only an attempt that actually waited pays for any of this**, so a send that never blocks costs nothing.

Both CURRENT rosters are consulted before any history, so moving an id between `allowed_thread_ids` and `allowed_channel_ids` reads as the reclassification it is rather than as a withdrawal. After that: a thread the transport has ever admitted and that is on neither roster is refused, which holds after a restart where a configured thread's type was never resolved; likewise a shared guild channel ever admitted and now on neither, because a thread promotion POSTs to the **parent** channel and so a guild id reaches the predicate too; a DM whose peer this process can name is decided on **that peer**; and **anything left is refused**. The pairing is learned wherever it is knowable: `create_dm_channel` knows it when the bot opens a DM, an authorized inbound DM names it, and so does an authorized DM interaction. Both inbound cases matter because the answer goes to the channel the message or press arrived on, without ever opening it. All of them record on the authorized path only, so a denied sender or presser cannot plant one.

That last arm is deliberate. A DM channel this process never opened - a destination read back from a persisted link - names nobody, and asking instead whether the roster admits ANYBODY would let one remaining peer authorize a different, revoked one. At a network egress boundary "cannot tell" reads as no. The cost is that an unattended proactive DM to such a channel is refused rather than delivered when it serves one of the ladder's waits; a caller that needs it to survive re-opens the DM through `create_dm_channel`, which establishes the pairing. A refusal that follows the admission history being truncated to stay within its cap is audited under its own reason, so a capacity symptom does not read as an operator withdrawal. A button press is re-authorized after its acknowledgement as well: the rosters are read once before the ack, the ack serves the ladder's waits, and the governance read after it is off-loop, so the user and thread rosters are read again before the press resolves anything. An explicit reject still lands, because a denial is what a withdrawal wants.
